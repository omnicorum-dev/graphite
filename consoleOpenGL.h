...
#ifndef CONSOLEOPENGL_H
#define CONSOLEOPENGL_H
...
class ConsoleOpenGL : public Console {
private:
...
public:
    ...
    void consoleRun(std::function<Graphite::Canvas&(f32)> gameUpdate) override {
...
static u32* front_buffer = nullptr;
static u32* back_buffer  = nullptr;
...
void init_buffers(int w, int h) {
    ...
    front_buffer = new std::uint32_t[w * h];
    back_buffer  = new std::uint32_t[w * h];

    std::memset(front_buffer, 0, w * h * sizeof(std::uint32_t));
    std::memset(back_buffer,  0, w * h * sizeof(std::uint32_t));
}
...
inline std::uint32_t* get_back_buffer() {
    return back_buffer;
}

inline std::uint32_t& back_pixel(int x, int y) {
...
void render_screen(GLuint shader, GLuint vao) {
    glClear(GL_COLOR_BUFFER_BIT);
         ...
         GLuint vao)
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // 4. DRAW
        render_screen(shader, vao);

        glfwSwapBuffers(window);
    }
}*/

#endif //CONSOLEOPENGL_H
